...
<head>
    ...
    <style>
        ...
        * {
        ...
        button.register-btn.loading .spinner {
            display: block;
        }
...
<div class="login-card" id="loginCard">
    ...
    <div class="illustration" id="illustration">
        ...
        <svg viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg" style="width:100%;">
            ...
            <g id="car-sedan" transform="translate(110, 160)">
                ...
                <animateTransform attributeName="transform" type="translate" values="110,160;110,157;110,160"
            ...
            <g id="car-suv" transform="translate(180, 80) scale(0.6)">
                ...
                <animateTransform attributeName="transform" type="translate" values="180,80;180,78;180,80"
    ...
    <div class="form-section">
        ...
        <p class="subtitle">Join the EZEV fleet.</p>

        <form action="RegisterServlet" method="post" id="registerForm" novalidate>

            <div class="input-group">
                <input type="text" name="user_name" id="nameInput" class="track-input" autocomplete="name">
                <label for="nameInput">Full Name</label>
                <div class="hint" id="nameHint"></div>
            </div>

            <div class="input-group">
                <input type="email" name="user_email" id="emailInput" class="track-input" autocomplete="email">
                <label for="emailInput">Email Address</label>
                <div class="hint" id="emailHint"></div>
            </div>

            <div class="input-group">
                <input type="tel" name="user_phone" id="phoneInput" class="track-input" autocomplete="tel">
                <label for="phoneInput">Phone Number</label>
                <div class="hint" id="phoneHint"></div>
            </div>

            <div class="input-group">
                <input type="password" name="user_password" id="password" autocomplete="new-password">
...
<script>
    ...
    const allTrackInputs = document.querySelectorAll('.track-input');
    ...
    nameInput.addEventListener('input', () => {
        ...
        if (!v) { setHint(nameHint, '', ''); return; }
        if (v.length < 2) { setHint(nameHint, 'Name is too short.', 'error'); return; }
        setHint(nameHint, '✓ Looks good!', 'ok');
    });

    emailInput.addEventListener('input', () => {
        const v = emailInput.value;
        if (!v) { setHint(emailHint, '', ''); emailInput.classList.remove('input-error', 'input-ok'); return; }
        const ok = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v);
        emailInput.classList.toggle('input-error', !ok);
        emailInput.classList.toggle('input-ok', ok);
        setHint(emailHint, ok ? '✓ Valid email!' : 'Enter a valid email address.', ok ? 'ok' : 'error');
    });

    phoneInput.addEventListener('input', () => {
        const v = phoneInput.value.replace(/\D/g, '');
        if (!v) { setHint(phoneHint, '', ''); return; }
        if (v.length < 10) { setHint(phoneHint, 'Enter a valid 10-digit number.', 'error'); return; }
    ...
    /* Clear hints on blur if empty */
    [nameInput, emailInput, phoneInput].forEach(inp => {
        inp.addEventListener('blur', () => {
            if (!inp.value.trim()) {
                const hintEl = document.getElementById(inp.id.replace('Input', '') + 'Hint');
    ...
    document.addEventListener('mousemove', e => {
        ...
        if (trackFieldFocused) return;
        movePupilsMouse(suvPupils, suvGroup, 6, e);
        movePupilsMouse(sedanPupils, sedanGroup, 4, e);
    });

    document.addEventListener('mouseleave', () => {
        if (!trackFieldFocused) setPupils(0, 0);
    });

    /* Text input → left-to-right eye scan */
    const MAX_CHARS = 30;
    const MAX_X = 7;

    function updateEyesForInput(input) {
        const pos = input.selectionStart ?? input.value.length;
        const ratio = Math.min(pos / MAX_CHARS, 1);
        const xOff = (ratio * 2 - 1) * MAX_X;
        const yOff = ratio * 2;
        setPupils(xOff, yOff);
    }

    allTrackInputs.forEach(input => {
        input.addEventListener('focus', () => {
            trackFieldFocused = true;
            stopBlink();
            updateEyesForInput(input);
        });
        input.addEventListener('input', () => updateEyesForInput(input));
        input.addEventListener('keyup', () => updateEyesForInput(input));
        input.addEventListener('click', () => updateEyesForInput(input));
        input.addEventListener('blur', () => {
            trackFieldFocused = false;
            setPupils(0, 0);
            startBlink();
        });
    });
    ...
    passwordInput.addEventListener('focus', () => {
        trackFieldFocused = false; // password doesn't track
    ...
    document.getElementById('registerForm').addEventListener('submit', function (e) {
        ...
        if (!nameInput.value.trim() || nameInput.value.trim().length < 2) {
            ...
            setHint(nameHint, 'Please enter your full name.', 'error');
            valid = false;
        }
        ...
        if (!phoneInput.value || phoneInput.value.replace(/\D/g, '').length < 10) {
            ...
            setHint(phoneHint, 'Please enter a valid phone number.', 'error');
            valid = false;
        }
