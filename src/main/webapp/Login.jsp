...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        h1 { font-size: 28px; margin-bottom: 5px; color: #2d3436; }
        ...
        button.login-btn {
            width: 100%; background-color: #00b894;
        ...
        /* ALERT BOX STYLE */
        .alert {
            padding: 10px; border-radius: 5px; font-size: 13px; text-align: center; margin-bottom: 15px;
        }
        .alert-error { background-color: #fab1a0; color: #d63031; }
        .alert-success { background-color: #55efc4; color: #006266; }

        /* ANIMATION */
        .pupil { transition: transform 0.1s ease-out; }
        .eyelid { transform-origin: top center; transform: scaleY(0); transition: transform 0.2s ease-in-out; }
        .illustration.blind .eyelid { transform: scaleY(1); }

        @media (max-width: 768px) {
            .login-card { flex-direction: column; }
            .illustration { width: 100%; height: 220px; }
...
<body>
    ...
    <div class="login-card">
        ...
        <div class="form-section">
            <h1>Welcome Back</h1>
            <p style="color:#636e72; margin-bottom:25px; font-size:14px;">Log in to manage your charging.</p>

            <%
                String msg = (String) session.getAttribute("message");
                if (msg != null) {
                    String alertClass = msg.contains("Success") ? "alert-success" : "alert-error";
            %>
            ...
            <form action="LoginServlet" method="post">
                ...
                <div style="display:flex; justify-content:space-between; font-size:12px; margin-bottom:20px; color:#636e72;">
                    <label><input type="checkbox"> Remember me</label>
                    <a href="#" style="color:#00b894; text-decoration:none;">Forgot password?</a>
                </div>
                
                <button type="submit" class="login-btn">Log In</button>
                
                <p style="text-align:center; font-size:12px; margin-top:20px; color:#b2bec3;">
                    New here? <a href="Register.jsp" style="color:#0984e3; font-weight:bold; text-decoration:none;">Create Account</a>
                </p>
            </form>
        </div>
    </div>
    ...
    <script>
        // Select fields with class 'track-input'
        ...
        trackInputs.forEach(input => {
            input.addEventListener('input', (e) => {
                ...
                // Calculate percentage (0 to 1) based on text length (max 30 chars)
                const percentage = Math.min(val / 30, 1);
                
                // Max movement range in pixels
                const moveRange = 10; 
                
                // Calculate offset: 0% = -5px (left), 100% = +5px (right)
                const xOffset = (percentage * moveRange) - (moveRange / 2);
                const yOffset = percentage * 3; // Slight look down as typing
            ...
            // Reset eyes to center when user clicks away
        ...
        // PRIVACY MODE (Close Eyelids on Password Focus)
